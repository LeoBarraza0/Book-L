...
class _CrearEjercicioScreenState extends State<CrearEjercicioScreen> {
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      ...List.generate(_preguntas.length, (i) => _buildPreguntaCard(i)),
      ...
      child: Column(
        ...
        children: [
          const Text('Información del ejercicio',
              style: TextStyle(fontFamily: 'Inter', fontWeight: FontWeight.w700, fontSize: 15, color: Color(0xFF676767))),
          const SizedBox(height: 16),
          _buildInputField(_tituloCtrl, 'Título', 'Ej: Cuestionario Lógico II'),
          const SizedBox(height: 14),
          _buildInputField(_descripcionCtrl, 'Descripción', 'Describe brevemente el ejercicio...', maxLines: 3),
        ],
      ),
    );
  }
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                ...
                Text('Pregunta ${index + 1}',
                    style: const TextStyle(fontFamily: 'Inter', fontSize: 16, fontWeight: FontWeight.w700, color: Colors.black87)),
                const Spacer(),
                if (_preguntas.length > 1)
                  GestureDetector(
                    onTap: () => setState(() { q.dispose(); _preguntas.removeAt(index); }),
                    child: Container(
                      width: 32, height: 32,
                      decoration: BoxDecoration(
            ...
            const SizedBox(height: 16),
            _buildInputField(q.contenidoCtrl, 'Enunciado', 'Escribe la pregunta...', maxLines: 2),
            const SizedBox(height: 14),
            _buildInputField(q.explicacionCtrl, 'Explicación (opcional)', 'Justificación de la respuesta...', maxLines: 2),
  ...
  Widget _buildMultipleChoiceOpciones(_QuestionData q) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Opciones', style: TextStyle(fontFamily: 'Inter', fontWeight: FontWeight.w600, fontSize: 13, color: Color(0xFF676767))),
        const SizedBox(height: 4),
        const Text('Toca el círculo para marcar la correcta', style: TextStyle(fontFamily: 'Inter', fontSize: 11, color: Color(0xFF999999))),
        const SizedBox(height: 12),
          ...
          return Padding(
            ...
            child: Row(
              children: [
                GestureDetector(
                  onTap: () => setState(() {
                    for (var op in q.opciones) { op.correcta = false; }
                    o.correcta = true;
                  }),
                  child: AnimatedContainer(
                    ...
                    child: Center(child: Text(
                      ...
                      style: TextStyle(fontFamily: 'Inter', fontWeight: FontWeight.w800, fontSize: 14,
                          color: o.correcta ? Colors.white : const Color(0xFF999999)),
                    )),
                  ),
                ),
                const SizedBox(width: 12),
                  ...
                  child: Container(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        hintText: 'Opción ${i < letters.length ? letters[i] : i + 1}',
  ...
  Widget _buildTrueFalseOpciones(_QuestionData q) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Respuesta correcta', style: TextStyle(fontFamily: 'Inter', fontWeight: FontWeight.w600, fontSize: 13, color: Color(0xFF676767))),
        const SizedBox(height: 12),
        Row(
                ...
                child: GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    child: Row(
                      ...
                      children: [
                        Icon(icon, size: 20, color: o.correcta ? _accentColor : const Color(0xFF999999)),
                        const SizedBox(width: 8),
                        Text(label, style: TextStyle(
                          fontFamily: 'Inter', fontWeight: FontWeight.w700, fontSize: 14,
                          color: o.correcta ? _accentColor : const Color(0xFF999999),
                        )),
                      ],
                    ),
                  ),
  ...
  Widget _buildRespuestaCortaOpciones(_QuestionData q) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Respuesta esperada', style: TextStyle(fontFamily: 'Inter', fontWeight: FontWeight.w600, fontSize: 13, color: Color(0xFF676767))),
        const SizedBox(height: 12),
        _buildInputField(q.opciones[0].ctrl, '', 'Escribe la respuesta correcta...', maxLines: 2),
      ],
    );
  }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.add_circle_outline_rounded, color: _accentColor, size: 22),
            const SizedBox(width: 8),
            Text('Añadir pregunta', style: TextStyle(
              fontFamily: 'Inter', fontWeight: FontWeight.w700, fontSize: 14, color: _accentColor,
            )),
          ],
        ),
      ),
        ...
        child: Row(
          ...
          children: [
            if (_guardando)
              ...
              const Text('Guardar Ejercicio', style: TextStyle(
                fontFamily: 'Inter', fontSize: 16, fontWeight: FontWeight.w700, color: Colors.white, letterSpacing: 0.3,
              )),
            ],
          ],
        ),
  ...
  // ── Input reutilizable ──────────────────────────────────────────────────────
  Widget _buildInputField(TextEditingController ctrl, String label, String hint, {int maxLines = 1}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        if (label.isNotEmpty) ...[
          Text(label, style: const TextStyle(fontFamily: 'Inter', fontWeight: FontWeight.w600, fontSize: 13, color: Color(0xFF676767))),
  ...
  void _guardar() {
    ...
    final preguntasInput = _preguntas.map((q) {
      ...
      return PreguntaInput(
        ...
        explicacion: q.explicacionCtrl.text.trim().isEmpty ? null : q.explicacionCtrl.text.trim(),
    ...
    EjerciciosController().crearEjercicioCompleto(
      ...
      descripcion: _descripcionCtrl.text.trim().isEmpty ? 'Ejercicio de ${widget.tipo.displayName}' : _descripcionCtrl.text.trim(),
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Row(children: [
          ...
          const SizedBox(width: 10),
          Expanded(child: Text(msg, style: const TextStyle(fontFamily: 'Inter', fontWeight: FontWeight.w600))),
        ]),
